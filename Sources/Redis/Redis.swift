...
public class Redis {
	...
	public func issue (command command : Command, withArguments commandArguments: CommandArguments) -> String? {
		...
		case .BITPOS(let key, let bit, let start_end):
			let stringArray = quoteItems(start_end)
		...
		case .LPUSH(let key, let values):
			let newValues = quoteItems(values)
		...
		case .SADD(let key, let members):
			let newValues = quoteItems(members)
			result = self.command("\(commandString) \(key) \(newValues.joined(separator:" "))")
		...
		case .SREM(let key, let members):
			let newMembers = quoteItems(members)
	...
	public func quoteItems(array: Array<String>) -> Array<String> {
		let result: [String] = array.flatMap({ String("\"\($0)\"") })
		return result
	}

	public func quoteItems(array: Array<Int>) -> Array<String> {
		let result: [String] = array.flatMap({ String("\"\($0)\"") })
		return result
	}
